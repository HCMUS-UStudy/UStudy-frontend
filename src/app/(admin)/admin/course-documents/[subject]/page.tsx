...
const CourseDocumentsPage = ({ params }: { params: Promise<Params> }) => {
  ...
  const filteredFolders = (
    foldersBySubject[subject as keyof typeof foldersBySubject] || []
  ).filter((folder) => {
    const matchesSearch =
      folder.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      folder.description.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesFilter = selectedFilter
      ? folder.name.toLowerCase().includes(selectedFilter.toLowerCase())
      : true;
  ...
  const handleAttachmentClick = (subject: string, chapter: string) => {
    return `/admin/course-documents/${encodeURIComponent(
      subject
    ...
    <div className="p-6 bg-gradient-to-b from-gray-50 to-blue-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-8 px-4">
        {paginatedFolders.map((folder) => (
          <div
            key={folder.id}
            className="relative flex items-center p-3 rounded-xl shadow-lg bg-white group">
