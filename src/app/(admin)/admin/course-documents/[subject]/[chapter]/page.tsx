...
import { FaFilePdf, FaFileWord, FaEye, FaDownload, FaEllipsisV } from 'react-icons/fa'; // Import new icons
    ...
    return (
      <div className="mb-4 text-gray-700">
        <span className="text-sm">
          ...
          {' > '}
          <a href={`/admin/course-documents/${subject}`} className="text-black hover:text-blue-600 hover:underline ml-2 mr-2">{subject}</a>
          {' > '}
          <a href={`/admin/course-documents/${subject}/${chapter}`} className="text-blue-600 hover:underline ml-2">{chapter}</a>
        </span>
      </div>
    );
  };
    ...
    <div className="p-6 bg-gradient-to-b from-gray-50 to-blue-50 min-h-screen">
        ...
        <div className='flex space-x-4'>
          ...
          {isSelectMode && (
            <div className="flex space-x-2">
              <Button onClick={handleSelectAll} className="bg-sky-400 text-white hover:bg-sky-500 transition">
                {allSelected ? 'Unselect All' : 'Select All'}
              </Button>
              <Button className="bg-red-500 text-white hover:bg-red-600 transition">Delete All</Button>
              <Button className="bg-green-500 text-white hover:bg-green-600 transition">Move All</Button>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {paginatedDocuments.map((doc, index) => (
          <div
            key={doc.name}
            className={`border p-4 rounded-lg shadow-md ${selectedDocuments.includes(doc.name) ? 'bg-blue-100' : 'bg-white'} hover:shadow-lg transition`}
          >
            ...
            <div className="flex justify-end space-x-2">
              <Button onClick={() => window.open(doc.url, '_blank')} className="bg-blue-500 text-white hover:bg-blue-600 transition p-2">
                <FaEye className="text-white text-sm" />
              </Button>
              <Button onClick={() => window.open(doc.url, '_blank')} className="bg-green-500 text-white hover:bg-green-600 transition p-2">
