...
export default function TeacherEnrollment({ classId }: { classId: string }) {
  ...
  const [registerTeachers, setRegisterTeachers] = useState<RegisterClassItem[]>(
    [],
  );
  const [classTeachers, setClassTeachers] = useState<MemberItem[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const [fetchRegisterTeachers, fetchClassTeachers] = await Promise.all([
        getListAvailableTea(classId, "", 0, 5),
        getListMembers(classId, "", 0, 5, "TEACHER"),
      ]);
      setRegisterTeachers(fetchRegisterTeachers.content);
      setClassTeachers(fetchClassTeachers.content);
    } catch (error) {
      console.log(error);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    if (selectAll) {
      setSelectedIds(registerTeachers.map((item) => item.id));
    } else {
      setSelectedIds([]);
    }
  }, [selectAll, registerTeachers]);
  useEffect(() => {
    if (selectedIds.length === registerTeachers.length) {
      setSelectAll(true);
    }
