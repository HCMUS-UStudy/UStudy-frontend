...
export const Notification = ({ role }: { role: string }) => {
  ...
  const unreadCount = useMemo(() => {
    return notifications.filter((item: NotificationItem) => !item.read && !readNotifications.has(item.id)).length;
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    if (scrollTop + clientHeight >= scrollHeight * 0.8 && hasMoreNotifications && !isLoadingMore) {
          ...
          >
            {isLoading ? (
              ...
              <div className="p-8 text-center">
                ...
                <div className="text-gray-400 text-sm mt-1">Bạn sẽ nhận được thông báo ở đây</div>
              </div>
            ) : (
              <div className="flex flex-col gap-2 p-3">
                {displayedNotifications.map((item: NotificationItem, idx: number) => {
                  // Tính số ngày trước
                  let daysAgoLabel = "";
                  if (item.sendDate) {
                  ...
                  let typeIcon = null;
                  if (item.receiverType === "CLASS") typeIcon = <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V7" /><path strokeLinecap="round" strokeLinejoin="round" d="M16 3H8a2 2 0 00-2 2v0a2 2 0 002 2h8a2 2 0 002-2v0a2 2 0 00-2-2z" /></svg>;
                  else if (item.receiverType === "SYSTEM") typeIcon = <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3" /></svg>;
                  else if (item.receiverType === "USER") typeIcon = <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5.121 17.804A13.937 13.937 0 0112 15c2.485 0 4.797.657 6.879 1.804M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                          <span className="font-semibold text-[15px] text-primary-darkest truncate leading-tight">{item.title}</span>
                        </div>
                        {item.content && (
                          <div className="text-xs text-gray-500 mt-0.5 line-clamp-2 leading-snug">{item.content}</div>
                        )}
                        <div className="flex items-center justify-end gap-1 mt-1">
                          <svg className="w-3 h-3 text-gray-300" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3"/><circle cx="12" cy="12" r="10"/></svg>
                          <span className="text-[11px] italic text-gray-400">{daysAgoLabel}</span>
                        </div>
                      </div>
                    </div>
                  );
