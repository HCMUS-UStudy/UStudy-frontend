...
export default function ClassCard({ cls, completed }: { cls: ClassTeacher, completed: boolean }) {
  ...
  const getTime = (classTimes: ClassTime[]) => {
    return classTimes[0].startTime.split(":").slice(0, 2).join(":") 
          + " - " 
  ...
  const details = [
    ...
    { icon: <FaRegClock className="font-semibold" />, text: getTime(cls.classTimes) },
    { icon: <GrScheduleNew className="font-semibold" />, text: getDay(cls.classTimes) },
    { icon: <MdPeopleOutline className="font-semibold" />, text: `${cls.students.length} học sinh` },
  ];
