...
const Notification = ({ classId }: { classId: string }) => {
  ...
  return (
    <div className="flex flex-col border border-gray-200 shadow-sm rounded-3xl">
      <div className="flex justify-between bg-white pt-6 pb-4 px-8 rounded-3xl">
      ...
      >
        {notifications.map((notification) => (
          <div
            key={notification.id}
            className="flex justify-between px-16 py-2 hover:bg-gray-100 items-center"
          >
            <p
              ...
              onClick={() => {
                ...
                router.push(
                  `/teacher/classes/${classId}/notification/${notification.id}?showDetail=${showDetail}`,
                );
              }}
            >
              {notification.title}
