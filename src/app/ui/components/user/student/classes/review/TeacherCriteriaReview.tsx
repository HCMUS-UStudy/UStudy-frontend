...
interface TeacherCriteriaReviewProps {
...
const TeacherCriteriaReview: React.FC<TeacherCriteriaReviewProps> = ({
  teacherCriteria,
  teacherId,
  ratings = {},
  onRate,
  ...
  return (
    <div>
      {teacherCriteria.map((category, catIdx) => (
        <div key={category.title} className="mb-8">
          ...
          <div className="space-y-4">
            {category.items.map((criteria, idx) => (
              <div
                key={`${teacherId}-${criteria.id}`}
              ...
              >
                ...
                <div className="flex-shrink-0">
                  {renderStars(ratings?.[criteria.id] ?? 0, (rating) =>
                    onRate(criteria.id, rating),
                  )}
                </div>
              </div>
            ))}
...
export default TeacherCriteriaReview;
