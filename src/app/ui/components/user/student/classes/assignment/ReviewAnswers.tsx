  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-64px)] bg-gray-50">
      <div className="flex-shrink-0 pt-8 px-6">
        <h3 className="text-3xl font-bold mb-6 text-primary-darkest text-center">
          📝 Xem lại câu trả lời
        </h3>
      </div>
                        ...
                        <>
                          ...
                          {submittedAnswers[q.id]?.files?.length ? (
                            <ul className="mt-2 space-y-1 text-sm text-blue-600 underline">
                              {submittedAnswers[q.id]?.files?.map(
                                (file, idx) => <li key={idx}>{file.name}</li>,
                              )}
                            </ul>
                          ) : null}
                        </>
      ...
      <div className="flex-shrink-0 py-4 px-6 border-t border-gray-200 bg-white">
        <div className="flex justify-center gap-6">
          <button
            className="px-6 py-2 rounded-full bg-gray-200 text-gray-800 shadow-md hover:bg-gray-300 transition-colors transform hover:scale-105"
