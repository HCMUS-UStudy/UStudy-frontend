...
import { FaBook, FaTasks, FaQuestionCircle, FaClipboardList } from "react-icons/fa";
...
export default function Home() {
    ...
    const homeworkList = [
        { id: 1, subject: "Toán học", icon: <FaClipboardList className="h-5 w-5 text-blue-500" />, progress: 80 },
        { id: 2, subject: "Lý học", icon: <FaClipboardList className="h-5 w-5 text-green-500" />, progress: 60 },
        { id: 3, subject: "Hóa học", icon: <FaClipboardList className="h-5 w-5 text-red-500" />, progress: 40 }
    ];

    const subjectScores = {
        labels: [
            "Toán học", "Lý học", "Hóa học", "Sinh học", "Ngữ văn",
            ...
            tooltip: {
                ...
                callbacks: {
                    ...
                    label: function (context: any) {
                        const studentScore = context.raw;
        ...
        <div className="flex flex-col md:flex-row min-h-screen bg-gradient-to-tr from-blue-50 via-white to-gray-100 rounded-3xl">
            ...
            <div className="flex-grow pr-6 md:pr-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                    {/* Tổng số lớp học */}
                    <Card className="rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow p-4 bg-white">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    ...
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-lg hover:shadow-xl transition-shadow">
                        ...
                        <ul className="space-y-4">
                            {homeworkList.slice(0, 3).map((homework) => ( // Display only the first 3 items
                                <li key={homework.id} className="flex items-center justify-between py-3 px-4 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                                    <div className="flex items-center">
                                        <div className="text-2xl text-blue-500">{homework.icon}</div>
                                        <p className="text-lg font-medium text-gray-700 ml-3">{homework.subject}</p>
                                    </div>
                                        ...
                                        <div className="w-32 bg-gray-200 h-2 rounded-full">
                                            <div
                                                ...
                                                style={{
                                                    ...
                                                    backgroundColor: homework.progress > 50 ? '#4CAF50' : '#FF6F61'
                                                }}
                                            />
                                        </div>
                                        <span className="text-sm font-medium text-gray-600">{homework.progress}%</span>
                                    </div>
                                </li>
                            ))}
                        </ul>
                    ...
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-lg hover:shadow-xl transition-shadow">
                        <div className="flex items-center justify-between mb-6" >
                            <h3 className="text-2xl font-semibold text-gray-800">Kết quả học tập</h3>
                            <button className="text-blue-600 font-semibold hover:text-blue-800 transition-colors">
                                Xem thêm
                            </button>
                        </div>
                ...
                {/* Schedule */}
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-lg hover:shadow-xl transition-shadow mt-6">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        {scheduleData.map((schedule, index) => (
                            ...
                            >
                                ...
                                <div className="flex flex-col space-y-1">
                                    <span className="text-lg font-semibold text-gray-800">{schedule.subject}</span>
                                    <span className="text-sm text-gray-600">{schedule.grade}</span>
                                </div>

                                {/* Ngày học và giờ học */}
                                <div className="text-right space-y-1">
                                    <span className="text-sm text-gray-500 mr-4">{schedule.date}</span>
                                    <span className="text-sm font-semibold text-gray-800">{schedule.time}</span>
                                    <div className="flex justify-end"> {/* This ensures the button aligns to the right */}
