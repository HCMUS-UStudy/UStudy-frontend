...
const QuizPage = () => {
  ...
  const [currentQuiz, setCurrentQuiz] = useState<QnA | null>(null);
  ...
  useEffect(() => {
    const fetchQuizData = async () => {
      try {
        const response = await getDetailQuiz(quizId as string);
  ...
  const handleFinishQuiz = async () => {
    ...
    const durationInMinutes = Math.round(
      (currentQuiz.duration * 60 - timeLeft) / 60,
    );
    const body = {
                ...
                <button
                  ...
                  onClick={() => {
                    if (
                      ...
                      handleSubmitQuiz(); // Gọi hàm nộp bài khi ở câu hỏi cuối cùng
                    } else {
                      setCurrentQuestionIndex((prev) =>
                        Math.min(prev + 1, currentQuiz.questions.length - 1),
                      );
